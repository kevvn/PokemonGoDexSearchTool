...
const RegionSection = React.memo(({ region, pokemons, selectedIds, togglePokemon, handleRegionSelection, isCollapsed, toggleCollapse }) => {
  // Optimization: use a manual loop instead of .filter().length to count selections.
  // This avoids allocating a new array just to count its size, saving memory and GC overhead.
  let selectedCount = 0;
